...
export class ViewPhotoPageComponent implements OnInit {
  ...
  constructor(private appStore: AppStore, private _router: Router, private activatedRoute: ActivatedRoute) {}

  ngOnInit(): void {
   
    const photoId = this.activatedRoute.snapshot.paramMap.get('id')?.split(':')[1];
